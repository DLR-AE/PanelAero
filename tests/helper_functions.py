    ...
    def compare_AICs(self, AIC_a, AIC_b, n):
        ...
        if not result_allclose:
            ...
            for i_row in range(n):  
                M_real.append(np.dot(AIC_b[i_row,:].real, AIC_a[i_row,:].real) / np.linalg.norm(AIC_b[i_row,:].real) / np.linalg.norm(AIC_a[i_row,:].real))
                M_imag.append(np.dot(AIC_b[i_row,:].imag, AIC_a[i_row,:].imag) / np.linalg.norm(AIC_b[i_row,:].imag) / np.linalg.norm(AIC_a[i_row,:].imag))        
